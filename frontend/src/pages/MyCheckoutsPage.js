...
import { getMyCheckouts, returnBook } from '../api';
...
export default function MyCheckoutsPage() {
  ...
  const fetchCheckouts = async () => {
    try {
      const res = await getMyCheckouts(activeOnly);
    ...
    <div className="page">
      ...
      {checkouts.length === 0 ? (
        <p style={{ textAlign: 'center', color: '#718096', marginTop: 40 }}>
          {activeOnly ? 'No active checkouts.' : 'No checkout history.'}
        </p>
      ) : (
          ...
          <tbody>
            {checkouts.map(c => (
              <tr key={c.id}>
                ...
                <td>
                  {c.returnedAt ? (
                    <span className="returned">Returned {new Date(c.returnedAt).toLocaleDateString()}</span>
                  ) : (
